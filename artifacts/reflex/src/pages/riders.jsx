...
export default function Riders() {
  ...
  const visible = riders.data?.filter((rider) => `${rider.name} ${rider.phone}`.toLowerCase().includes(search.toLowerCase())) ?? [];

  return (
    <div className="page-wrap">
      <div className="page-heading"><div><div className="eyebrow">People / Field team</div><h1 className="page-title">Rider roster</h1><p className="page-description">Know who is ready, who is moving, and who needs a quieter shift.</p></div><div className="connection-pill"><Users size={13} /> {riders.data?.length ?? '—'} riders</div></div>
      <div className="panel" style={{ marginBottom: 18 }}><div className="table-tools"><div className="search-wrap"><Search size={15} /><input className="input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find a rider" data-testid="input-search-riders" /></div></div></div>
